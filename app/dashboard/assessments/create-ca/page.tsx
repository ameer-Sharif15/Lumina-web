...
export default function CreateCATestPage() {
  ...
  const CA_TYPES = [
    { key: "ca_1", label: t("ca_1") || "CA 1" },
    { key: "ca_2", label: t("ca_2") || "CA 2" },
    { key: "ca_3", label: t("ca_3") || "CA 3" },
  ];
  ...
  const [selectedCAType, setSelectedCAType] = useState(CA_TYPES[0]);

  // Assessment Parameters
  const [availableTopics, setAvailableTopics] = useState<string[]>([]);
  const [fetchingTopics, setFetchingTopics] = useState(false);
  const [selectedTopics, setSelectedTopics] = useState<string[]>([]);
  const [format, setFormat] = useState("mixed"); // mixed, mcq, theory
  ...
  const handleGenerate = async () => {
    ...
    handleAIAction({
      ...
      body: {
        classroomId: selectedClass._id,
        subjectName: selectedSubject,
        title: `${selectedSubject} - ${selectedCAType.label}`,
        type: selectedCAType.label,
        term: selectedTerm.label,
        format,
      ...
      successMessage: t('assessment_preview') || 'Assessment generated successfully!',
    });
  };
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {t("ca_intro") || "Create Continuous Assessments (CA) with AI-generated questions aligned to your class topics."}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Assessment Sequence Selector */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
              <Settings size={14} className="text-emerald-600" />
              {t("assessment_sequence") || "Assessment Type"}
            </label>
            <select
              value={selectedCAType.key}
              onChange={(e) => {
                const found = CA_TYPES.find(c => c.key === e.target.value);
                if (found) setSelectedCAType(found);
              }}
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 px-4 text-sm font-bold text-slate-700 outline-none appearance-none cursor-pointer focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/25 transition-all"
            >
              {CA_TYPES.map((type) => (
        ...
        {/* Dynamic Topic Syllabus Checklist */}
        <div className="border-t border-slate-100 pt-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                <ListTodo size={14} className="text-emerald-600" />
                {t("topics_to_cover") || "Topics to Cover"}
              </label>
              <p className="text-[11px] text-slate-400 font-semibold mt-0.5">
                {fetchingTopics ? "Fetching topics from classroom database..." : `${selectedTopics.length} selected`}
              </p>
            </div>
            
            {/* Quick selections */}
            {!fetchingTopics && availableTopics.length > 0 && (
          ...
          {fetchingTopics ? (
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-2xl py-8 px-4 text-center">
              ...
              <h4 className="text-sm font-extrabold text-slate-700">{t("no_lesson_plans") || "No lesson plans found for this selection"}</h4>
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                <input
                  type="text"
                  placeholder={t("search_topics") || "Search topics to cover..."}
                  value={topicSearch}
        ...
        <div className="border-t border-slate-100 pt-6 space-y-3">
          <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block">
            {t("question_format") || "Question Format"}
          </label>
          <div className="grid grid-cols-3 gap-3">
            {[
              { id: "mixed", label: t("mixed") || "Mixed", desc: "MCQ + Theory" },
              { id: "mcq", label: t("mcq") || "MCQ Only", desc: "Single option choices" },
              { id: "theory", label: t("theory") || "Theory Only", desc: "Subjective questions" },
            ].map((item) => (
              <button
                key={item.id}
        ...
        <div className="border-t border-slate-100 pt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wide block">
              {t("total_questions") || "Total Questions"}
            </label>
            <p className="text-[11px] text-slate-400 font-semibold mt-0.5">Adjust CA paper question counts</p>
          </div>
        ...
        <div className="border-t border-slate-100 pt-6">
          <button
            ...
            disabled={isAILoading || selectedTopics.length === 0}
            className={`w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-sm py-4 rounded-2xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer ${
              (isAILoading || selectedTopics.length === 0) ? "opacity-60 cursor-not-allowed" : ""
            }`}
          >
            {isAILoading ? (
              ...
              <>
                ...
                <span>{t("generate_assessment") || "Generate Assessment Paper"}</span>
              </>
            )}
          </button>
        </div>
