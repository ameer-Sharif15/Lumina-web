...
export default function EditClassPage() {
  ...
  // Load existing dynamic class data from database
  const fetchClassDetails = useCallback(async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setStudentCount(classData.studentCount?.toString() || '');
        
        const termToSet = TERMS.includes(classData.term) ? classData.term : TERMS[0];
        setSelectedTerm(termToSet);

        // Map assigned subjects
        const ids: string[] = [];
        const custom: any[] = [];
        (classData.subjects || []).forEach((s: string) => {
          const found = initialSubjects.find(is => is.name === s);
          if (found) {
            ids.push(found.id);
          } else {
            const newSub = { id: Math.random().toString(), name: s };
            custom.push(newSub);
            ids.push(newSub.id);
          }
        });
        ...
        // Map weekly periods schedule
        const newSched: Record<string, any[]> = {
          MON: [], TUE: [], WED: [], THU: [], FRI: [], SAT: [], SUN: [],
        };
        (classData.schedule || []).forEach((p: any) => {
          if (newSched[p.day]) {
            newSched[p.day].push({
      ...
      console.error('Error fetching class details for editing:', err);
    } finally {
      setIsLoading(false);
    }
  }, [id]);
  ...
  // Submit edit class payload to backend
  const handleUpdateClass = async () => {
    if (!className || !academicYear || isSubmitting) return;
    setIsSubmitting(true);

    try {
      // Flatten schedule for the backend schema
  ...
  // Delete class from system securely
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto min-h-screen text-slate-800">
      ...
      <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="w-10 h-10 rounded-full border border-red-100 flex items-center justify-center hover:bg-red-50 text-red-500 transition-colors cursor-pointer"
            title="Delete Class"
          >
            <Trash2 size={18} />
          </button>
      ...
      {/* Delete Class Confirmation Overlay Modal */}
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6 border border-slate-100 animate-in zoom-in-95 duration-150">
            <div className="w-12 h-12 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center mb-4">
              <AlertTriangle className="w-6 h-6" />
            </div>
            
            <h3 className="text-lg font-black text-slate-900 mb-2">Delete Class?</h3>
            <p className="text-sm text-slate-500 leading-relaxed mb-6 font-semibold">
              Are you sure you want to delete <strong className="text-slate-800">"{className}"</strong>? This will permanently erase the class and all its associated timetable slots. This action cannot be undone.
            </p>
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isDeleting}
                className="px-5 py-2.5 rounded-full text-xs font-bold bg-red-500 hover:bg-red-600 text-white flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                {isDeleting ? (
                  <>
